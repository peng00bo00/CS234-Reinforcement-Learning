...
def build_mlp(
  ...
  with tf.variable_scope(scope):
      ...
      out = tf.layers.dense(x, output_size, output_activation)
  return out# TODO
  #######################################################
  #########          END YOUR CODE.          ############
...
class PG(object):
  ...
  def add_placeholders_op(self):
    ...
    self.observation_placeholder = tf.placeholder(shape=[None, self.observation_dim], name="observation", dtype=tf.float32) # TODO
    if self.discrete:
      self.action_placeholder = tf.placeholder(shape=[None], name="action", dtype=tf.int32) # TODO
    else:
      self.action_placeholder = tf.placeholder(shape=[None, self.action_dim], name="action", dtype=tf.float32) # TODO

    # Define a placeholder for advantages
    self.advantage_placeholder = tf.placeholder(shape=[None], name="advantage", dtype=tf.float32) # TODO
    #######################################################
    #########          END YOUR CODE.          ############
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
      action_logits = build_mlp(mlp_input = self.observation_placeholder,
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    self.baseline = build_mlp(mlp_input = self.observation_placeholder,
                              ...
                              size = self.config.layer_size,
                              output_activation=None)
    self.baseline = tf.squeeze(self.baseline)# TODO
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      returns = [np.sum(np.power(self.config.gamma, np.arange(T - t)) * rewards[t:]) for t in range(T)]# TODO
      #######################################################
      #########          END YOUR CODE.          ############
