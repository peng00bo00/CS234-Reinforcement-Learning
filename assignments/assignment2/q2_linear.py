...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s          = tf.placeholder(tf.uint8,   shape=(None, state_shape[0], state_shape[1], config.state_history*state_shape[2]))
        self.a          = tf.placeholder(tf.int32,   shape=(None, ))
        self.r          = tf.placeholder(tf.float32, shape=(None, ))
        self.sp         = tf.placeholder(tf.uint8,   shape=(None, state_shape[0], state_shape[1], config.state_history*state_shape[2]))
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse=reuse) as _:
            ...
            out     = tf.layers.dense(flatten, num_actions, activation=None)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        q_collections           = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=q_scope)
        target_q_collections    = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=target_q_scope)
        self.update_target_op   = tf.group(*[tf.assign(t_var, q_var) for q_var, t_var in zip(q_collections, target_q_collections)])
    ...
    def add_loss_op(self, q, target_q):
        ...
        q_new = tf.reduce_sum(q * tf.one_hot(self.a, num_actions), axis=1)
